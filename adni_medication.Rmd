...
# Ensure that `medication_with_keymed_updated` is properly loaded before the join
...
cl_cognitive_marker <- cl_cognitive_marker %>%
  left_join(medication_with_keymed_updated %>% 
              ...
              select(rid, viscode, phase, keymed, all_medications, medication_category, visdate),
            by = c("rid", "viscode", "phase")) %>% 
  # Reorder columns, making sure to keep all desired columns
  select(rid, viscode, phase, keymed, all_medications, medication_category, examdate, visdate, everything())
```



```{r}
# Remove duplicate entries based on 'rid', 'viscode', and 'phase'
...
# Filter rows where the difference between examdate and visdate is more than one month
cognitive_marker_filtered <- cognitive_marker_date %>%
  filter(abs(interval(visdate, examdate) %/% months(1)) > 1)
```
