...
filtered_longitudinal_data_clean <- filtered_longitudinal_data_clean %>%
  mutate(time_since_baseline = round(as.numeric(difftime(examdate, baseline_examdate, units = "days")/365)))%>%
  select(rid,viscode2,time_since_baseline,phase,examdate,everything())# time passed in month
...
model_lme <- lme(
  phc_mem ~ time_since_baseline * phc_diagnosis + phc_race + phc_ethnicity +apoe4 + phc_education + phc_sex + age_baseline,
...
ggplot(plot_data, aes(x = time_since_baseline, y = predicted_phc_mem)) +
  geom_point()+
  geom_line(color = "blue", size = 1.2) +
  labs(title = "Fixed Effects - Model Prediction",
       x = "Time Since Baseline (Years)",
...
ggplot(plot_data, aes(x = time_since_baseline, y = predicted_phc_mem, color = factor(phc_diagnosis))) +
  geom_line(size = 1.2) +
  labs(title = "Predicted PHC Memory Score Over Time by PHC Diagnosis Group",
       x = "Time Since Baseline (Years)",
       y = "Predicted PHC Memory Score",
       color = "PHC Diagnosis") +
...
cross_sectional_model <- lm(phc_mem ~ phc_diagnosis + phc_race + apoe4 + phc_education + phc_sex + age_baseline+phc_ethnicity, 
...
cross_sectional_data <- cross_sectional_data %>%
  mutate(predicted_phc_mem = predict(cross_sectional_model, newdata = cross_sectional_data))


# Plot actual data points and predicted regression lines by phc_diagnosis
ggplot(cross_sectional_data, aes(x = age_baseline, y = phc_mem ))+
  geom_smooth(method = "loess", se = FALSE, size = 1.2) + 
  labs(title = "Cross-Sectional Model: Predicted PHC Memory Score by Baseline Covariates",
...
```{r}
# Check mean and maximum value of age_baseline
summary_stats <- filtered_longitudinal_data_clean_no_na %>%
  summarise(
    min_age_baseline = min(age_baseline),
    mean_age_baseline = mean(age_baseline), 
    max_age_baseline = max(age_baseline)  
  )

print(summary_stats)

# Trend for age 55-65 and age 65-75
new_data_age_55 <- filtered_longitudinal_data_clean_no_na[filtered_longitudinal_data_clean_no_na$age_baseline >= 55 & 
                                                              filtered_longitudinal_data_clean_no_na$age_baseline <= 65, ]

new_data_age_65 <- filtered_longitudinal_data_clean_no_na[filtered_longitudinal_data_clean_no_na$age_baseline >= 65 & 
                                                              filtered_longitudinal_data_clean_no_na$age_baseline <= 75, ]

# Generate predicted values for each dataset using the longitudinal model
new_data_age_55$predicted_phc_mem <- predict(model_lme, newdata = new_data_age_55)
new_data_age_65$predicted_phc_mem <- predict(model_lme, newdata = new_data_age_65)


ggplot(filtered_longitudinal_data_clean_no_na, aes(x = time_since_baseline)) +
    geom_line(data = new_data_age_65, aes(y = predicted_phc_mem, linetype = "Age 65-75"), color = "red", size = 1)+

  geom_point(aes(y = phc_mem, color = as.factor(phc_diagnosis)), alpha = 0.5, size = 1.5) +

  geom_line(data = new_data_age_55, aes(y = predicted_phc_mem, linetype = "Age 55-65"), color = "blue", size = 1) +
  scale_color_manual(name = "PHC Diagnosis",
                     values = c("1" = "green", "2" = "orange", "3" = "red"),
                     labels = c("1" = "Normal cognition", "2" = "Mild cognitive impairment", "3" = "Alzheimer's disease")) +
  scale_linetype_manual(name = "Baseline Age",
                        values = c("Age 55-65" = "solid", "Age 65-75" = "dashed")) +
  labs(title = "Longitudinal Trends of PHC_MEM for Different Baseline Ages",
       x = "Time Since Baseline (Years)",
       y = "PHC_MEM",
       color = "PHC Diagnosis",
       linetype = "Baseline Age") +
  theme_minimal()
```
